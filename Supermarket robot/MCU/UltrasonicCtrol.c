...
void UltraSoundInit(void)	
{
	...
	GPIO_InitStructure.GPIO_Pin = Echo_PINA;
	GPIO_Init(Echo_GPIOA , &GPIO_InitStructure);
	
	GPIO_InitStructure.GPIO_Pin = Echo_PINB;
	GPIO_Init(Echo_GPIOB , &GPIO_InitStructure);
	...
	GPIO_InitStructure.GPIO_Pin = Trig_PINA;
	GPIO_Init(Trig_GPIOA , &GPIO_InitStructure);
	
	GPIO_InitStructure.GPIO_Pin = Trig_PINB;
	GPIO_Init(Trig_GPIOB , &GPIO_InitStructure);
	...
	GPIO_EXTILineConfig(Echo_PINSOURCEA , Echo_PINSOURCEA);//将GPIO口配置成外部中断 
	EXTI_InitStructure.EXTI_Line = Echo_EXITLINEA;//将对应的GPIO口连接到中断线上
	EXTI_InitStructure.EXTI_Trigger = EXTI_Trigger_Falling;//中断事件类型，下降沿
	EXTI_InitStructure.EXTI_Mode = EXTI_Mode_Interrupt;//选择模式，中断型
	EXTI_InitStructure.EXTI_LineCmd = DISABLE;//使能该中断
	EXTI_Init(&EXTI_InitStructure);//将配置好的参数写入寄存器
	...
	GPIO_EXTILineConfig(Echo_PINSOURCEB , Echo_PINSOURCEB);//将GPIO口配置成外部中断 
	EXTI_InitStructure.EXTI_Line = Echo_EXITLINEB;//将对应的GPIO口连接到中断线上
	EXTI_InitStructure.EXTI_Trigger = EXTI_Trigger_Falling;//中断事件类型，下降沿
	EXTI_InitStructure.EXTI_Mode = EXTI_Mode_Interrupt;//选择模式，中断型
	EXTI_InitStructure.EXTI_LineCmd = DISABLE;//使能该中断
	EXTI_Init(&EXTI_InitStructure);//将配置好的参数写入寄存器
